...
import { ChevronDownIcon, PencilIcon, ChevronUpIcon } from '@heroicons/react/24/solid'
    ...
    return (
        <>  
            <div className="w-3/4 py-6 flex flex-row items-center justify-evenly">
                ...
                <p onClick={()=>handleViewClick()} className='hover:cursor-pointer'>See Project Details{isViewingDetails ? <ChevronUpIcon className='h-6 w-6' /> : <ChevronDownIcon className='h-6 w-6' />}</p>
            </div>
            {isViewingDetails ? (
                    <>
                ...
                {project.dayDetails.map((day, index)=>{
                   ...
                   <div className="w-3/4 py-6 flex flex-row items-center justify-between shrink-0 grow-0">
                        <div className='flex flex-col justify-center items-center'>
                            <h5 className='text-l font-bold text-gray-700'>Day {index+1}</h5>
